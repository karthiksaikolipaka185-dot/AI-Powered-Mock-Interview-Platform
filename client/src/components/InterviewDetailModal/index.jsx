...
const InterviewDetailModal = ({ interviewId, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
                ...
                <div className="p-6 border-b border-slate-200 dark:border-slate-800 glass flex items-center justify-between sticky top-0 z-10">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500">ID: {interviewInfo?.id} • Role: {interviewInfo?.role}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            className="flex items-center gap-1 text-xs font-bold bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 px-3 py-2 rounded-xl transition-colors"
                        >
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    {loading ? (
                        <div className="h-64 flex flex-col items-center justify-center gap-3">
                            ...
                            <p className="text-xs text-slate-500 font-medium">Fetching interview transcript & evaluation...</p>
                        </div>
                    ) : error ? (
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="glass p-4 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 shadow-sm flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-xl bg-slate-100 dark:bg-slate-800 text-primary-600 flex items-center justify-center font-bold overflow-hidden">
                                    ...
                                    <div className="space-y-0.5">
                                        <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100">{candidateInfo?.name}</h3>
                                        <p className="text-xs text-slate-500 flex items-center gap-1"><Mail size={12} /> {candidateInfo?.email}</p>
                                        {resumeSummary && (
                                ...
                                <div className="glass p-4 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 shadow-sm grid grid-cols-2 gap-2 text-xs">
                                    <div>
                                        <span className="text-slate-400 block">Target Role:</span>
                                        <strong className="text-slate-800 dark:text-slate-200 capitalize">{interviewInfo?.role}</strong>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <span className="text-slate-600 dark:text-slate-400">{new Date(interviewInfo?.startedAt).toLocaleTimeString()}</span>
                                    </div>
                                    <div>
                            ...
                            <div className="glass p-5 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 shadow-sm space-y-4">
                                ...
                                <div className="space-y-3 max-h-80 overflow-y-auto pr-2">
                                    {messages && messages.length > 0 ? (
                                        messages.map((m, idx) => (
                                            <div 
                                                ...
                                                className={`p-3 rounded-2xl text-xs space-y-1 ${
                                                    m.role === 'ai'
                                                    ? 'bg-blue-500/10 border border-blue-500/20 text-slate-800 dark:text-slate-200 mr-8'
                                                    : 'bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200 ml-8'
                                                }`}
                                            >
                                        ...
                                        <p className="text-xs text-slate-400 italic">No message transcript recorded.</p>
                                    )}
                                </div>
                            </div>
                                    ...
                                    {codeSubmissions.map(sub => (
                                        <div key={sub.id} className="space-y-3">
                                            <div className="flex items-center justify-between text-xs">
                                                <span className="font-bold text-slate-800 dark:text-slate-200">Programming Language: <strong className="text-amber-600 capitalize">{sub.language}</strong></span>
                                                <span className="text-slate-400">{new Date(sub.timestamp).toLocaleTimeString()}</span>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                                ...
                                                <div className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-center col-span-2 sm:col-span-1">
                                                    <span className="text-[10px] font-bold text-slate-400 uppercase block flex items-center justify-center gap-1">
                                            ...
                                            <div className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs space-y-1">
                                                <strong className="text-slate-800 dark:text-slate-200 block">AI Code Evaluation Notes:</strong>
                                                <p className="text-slate-600 dark:text-slate-400">{sub.aiEvaluation}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                ...
                <div className="p-4 border-t border-slate-200 dark:border-slate-800 glass flex justify-end">
                    <button
                        onClick={onClose}
                        className="px-5 py-2 bg-slate-900 hover:bg-slate-800 text-white dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white text-xs font-bold rounded-xl transition-all"
                    >
                        Close Modal
                    </button>
                </div>
